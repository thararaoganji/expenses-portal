...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            jwtAuthLogger.debug("No JWT token found in request to: {}", requestPath);
        ...
        try {
            ...
            jwtAuthLogger.debug("JWT token found for user: {} accessing: {}", username, requestPath);
        } catch (JwtException e) {
            // If the token is invalid (e.g., expired, malformed), send a 403 response.
            jwtAuthLogger.warn("Invalid or expired JWT token for request to: {} - Error: {}", requestPath, e.getMessage());
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) { //NOSONAR
            try {
                ...
                if (jwtUtil.validateToken(jwt, userDetails)) {
                    ...
                    jwtAuthLogger.debug("Authentication successful for user: {} with roles: {}", username, userDetails.getAuthorities());
                } else {
                    jwtAuthLogger.warn("JWT token validation failed for user: {}", username);
                }
            } catch (Exception e) {
                jwtAuthLogger.error("Error during authentication for user: {} - Error: {}", username, e.getMessage());
            }
        }
